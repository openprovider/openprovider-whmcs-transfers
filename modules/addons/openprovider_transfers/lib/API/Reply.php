...
class Reply
{
    ...
    protected function _parseReply($str = '')
    {
        ...
        $arr = APITools::convertXmlToPhpArray($str);
    ...
    public function getRaw()
    {
        ...
        $descNode->appendChild(
                $dom->createTextNode(mb_convert_encoding($this->faultString, Config::$encoding))
        );
        $dataNode = $replyNode->appendChild($dom->createElement('data'));
        APITools::convertPhpObjToDom($this->value, $dataNode, $dom);
        if (0 < count($this->warnings))
        {
            $warningsNode = $replyNode->appendChild($dom->createElement('warnings'));
            APITools::convertPhpObjToDom($this->warnings, $warningsNode, $dom);
        }
        return $dom->saveXML();
    }
